...
/// Rendered inside the elements ZStack at zIndex(-1).
/// Draws all connector lines, arrowheads, the live preview line,
/// anchor dots in connect mode, and the selection badge.
struct ConnectorOverlayView: View {
    ...
    let canvasID: UUID
    /// Canvas scale passed in so we can counter-scale UI chrome
    let canvasScale: CGFloat

    var body: some View {
        ZStack {
            // ── Background drag tracker for live preview ───────────
            // Only active in pickingTo state. Sits under everything so
            // it never steals taps from anchor dots or elements.
                ...
                Color.clear
                    ...
                    .gesture(
                        DragGesture(minimumDistance: 0)
                            .onChanged { value in
                                // Only update preview when we have a source locked in
            ...
            ForEach(connectors) { connector in
                if let (start, end) = ConnectorViewModel.resolvePoints(
                    ...
                    // Hit area — always 44pt in screen space regardless of zoom
            ...
            // ── Anchor dots — rendered on TOP of everything ────────
            // Must be last in ZStack so they're never covered.
            if vm.isConnectModeActive {
    ...
    private func anchorDots(for bounds: ElementBounds) -> some View {
        ForEach(ConnectorAnchor.allCases, id: \.rawValue) { anchor in
            ...
            ZStack {
                // Large invisible tap target — 44pt minimum regardless of zoom
                // Counter-scaled so 44pt in screen space = 44/canvasScale in canvas space
                    ...
                    .overlay(
                        Circle()
                            .strokeBorder(Color.accentColor,
                                          lineWidth: 2.5 / canvasScale)
                            .opacity(isFromAnchor ? 0 : 1)
                    )
                    .shadow(color: .black.opacity(0.25), radius: 4 / canvasScale)
            }
            .position(x: point.x, y: point.y)
            // highPriorityGesture wins over any parent gesture (pan, magnify, etc.)
            .highPriorityGesture(
                TapGesture()
                    .onEnded {
                        vm.selectAnchor(
    ...
    private func connectorBadge(connector: ConnectorModel, at point: CGPoint) -> some View {
        HStack(spacing: 6) {
            ...
            // Color menu
            Menu {
                ForEach(["primary","blue","red","green","orange","purple","pink","teal","gray"],
                        id: \.self) { name in
                    Button {
                        vm.updateStyle(connector: connector, colorName: name, context: context)
                    } label: {
                        HStack {
                ...
                Circle()
                    .fill(paletteColor(connector.colorName))
            ...
            Menu {
                ForEach([1.0, 2.0, 3.0, 4.0, 6.0], id: \.self) { w in
                    Button {
                        vm.updateStyle(connector: connector, strokeWidth: w, context: context)
                    } label: { Text("\(Int(w))pt") }
                }
            } label: {
                HStack(spacing: 2) {
        ...
        // Counter-scale so badge is always readable
    ...
    private func midPoint(start: CGPoint, end: CGPoint,
        ...
        case .curved:
            ...
            let t: CGFloat = 0.5
            let x = pow(1-t,3)*start.x + 3*pow(1-t,2)*t*cp1.x
                  + 3*(1-t)*t*t*cp2.x + t*t*t*end.x
            let y = pow(1-t,3)*start.y + 3*pow(1-t,2)*t*cp1.y
                  + 3*(1-t)*t*t*cp2.y + t*t*t*end.y
    ...
    private func tipStartPoint(start: CGPoint, end: CGPoint,
                                ...
                                strokeWidth: Double) -> CGPoint {
        let offset = CGFloat(strokeWidth * 5)
        switch style {
        case .straight:
            let angle = atan2(end.y - start.y, end.x - start.x)
            return CGPoint(x: end.x - offset * cos(angle),
